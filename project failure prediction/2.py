...
def run_pipeline(file_path):
    ...
    # Ensemble
    ens_probs = (cb_probs + xgb_probs) / 2

    # 3. حساب المقاييس والرسومات
    all_probs = {'CatBoost': cb_probs, 'XGBoost': xgb_probs, 'Ensemble': ens_probs}
    results = []

    # إعداد الرسومات المجمعة
    ...
    plt.title('Top 15 Features Importance (CatBoost)')
    plt.tight_layout()
    plt.savefig(f'{OUTPUT_DIR}/feature_importance.png')
    plt.close()

    # 5. توزيع احتمالات التنبؤ (Prediction Probability Distribution)
    plt.figure(figsize=(10, 6))
    for name, probs in all_probs.items():
        sns.kdeplot(probs, label=name, fill=True, alpha=0.3)
...
if __name__ == "__main__":
    ...
    results_df.to_csv(f'{OUTPUT_DIR}/final_metrics_report.csv', index=False)
